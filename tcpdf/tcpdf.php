...
public function Header() {
		if ($this->header_xobjid === false) {
            ...
            $this->Image('@'.file_get_contents('./logo.png'),10,6,0,13);
            $this->SetFont('helvetica', '', 9);
          $this->Write(0, 'W: www.xyma.in  M: info@xyma.in', '', 0, 'R', true, 0, false, false, 0);
        $this->setCellHeightRatio(2.3);
...
public function Footer() {
		...
		if ($this->getRTL()) {
			...
			$this->Cell(0, 0, $logo, 'T', 0, 'C');
            $this->SetX($this->original_rMargin);
			$this->Cell(0, 0,  $currentTime , 'T', 0, 'R');
		} else {
			$this->SetX($this->original_lMargin);
			$this->Cell(0, 0, $this->getAliasRightShift().$logo, 'T', 0, 'C');
            $this->SetX($this->original_rMargin);
			$this->Cell(0, 0,  $currentTime , 'T', 0, 'R');
		}
	}
...
$ImageH = 60;
    ...
    $myX = ( $myPageWidth / 2 ) - 35;
    $myY = ( $myPageHeight / 2 ) -80;

        $this->SetAlpha(0.09);

    $this->Image(K_PATH_IMAGES.'pdfback.png', $myX, $myY, $ImageW, $ImageH, '', '', '', true, 150);
